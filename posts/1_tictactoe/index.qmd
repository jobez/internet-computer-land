...
Do I need to write text?

```{ojs}
//| panel: sidebar
//| echo: false

import { connectWallet  } from "/posts/0_hello_wallet/index.qmd";
d3 = require("d3@6");

viewof wallet = Inputs.button("Connect", {value: null, reduce: connectWallet})

viewof game_id = Inputs.range([0, 255], {step: 1, label: "Game Id"})


viewof counter = Inputs.button([
  ["Create", value => (e) => console.log("create")],
  ["Join", value => (e) => console.log("join")]
...
function handleTurn(state, col, row) {
  if (state.board[col][row] === 0) {
    if (state.turn % 2 === 0) {
      ...
      state.x_player_state = state.x_player_state + cr_to_binary(col, row)
    } else {
      state.board[col][row] = O_PLAYER;
      state.o_player_state = state.o_player_state + cr_to_binary(col, row)      
    }
    state.turn++;
    // winGame(state);
...
createState = () => {
  ...
  //state[2][2] = 1;
  return { board, turn: 0, winner: 0, x_player_state: 0, o_player_state: 0 };
}
```
