...
operative_state = {
    ...
    let player_role = getRole(c_game_state);    
    let shouldMove = ourTurn(player_role, c_game_state);
...
```{ojs}
function ourTurn(role, game_state) {
    var last_mover = game_state.last_mover
    console.log("here", last_mover, role);    
    if (role == "X") {
...
```{ojs}
async function* renderGame(contractState) {
...
```{ojs}
function renderBox(cr, cellValue, onClick) {
  if (operative_state) {  
    let contractState = operative_state[1];
    let shouldMove = operative_state[2];
    let xState = contractState.state_x;
    let oState = contractState.state_o;
    let binaryRep = cr_to_binary(cr[0], cr[1]);
    let maybeX = xState & binaryRep;
    let maybeO = oState & binaryRep;
    console.log(cr, binaryRep, maybeX, xState, maybeO, oState);
    if (maybeX == binaryRep) {
        cellValue = 1;
    }
    if (maybeO == binaryRep) {
  ...
  <div style="display:flex; flex-direction: column; justify-content: center; align-items: center;">
    ...
    ">${d3.range(GRID_WIDTH).map((row) => {
      return html`<div>${d3.range(GRID_HEIGHT).map((col) =>
        renderBox([col, row], state.board[col][row], () => {
